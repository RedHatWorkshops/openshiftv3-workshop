...
rectangle. The sample app can be browsed at http://gogs-ci.{{APPS_ADDRESS}}/RedHatWorkshops/bluegreen.git
...
....
$ oc new-app --image-stream=php --code=http://gogs-ci.{{APPS_ADDRESS}}/RedHatWorkshops/bluegreen.git --env COLOR=blue --name=blue
....
...
....
$ oc new-app --image-stream=php --code=http://gogs-ci.{{APPS_ADDRESS}}/RedHatWorkshops/bluegreen.git --env COLOR=green --name=green
....
